 ...
 */
public class SaldoVacacionesCenicco implements ProcesoDAO<SaldoVacaciones> {
...
//        
        ...
        Parametro fechaDescanso = ControladorWS.getInstance().getParametro(Parametros.FECHAS_DESCANSO);
        List<Date> diasFeriados = new ArrayList<>();
        SimpleDateFormat formatoDiaFeriado = new SimpleDateFormat("yyyy-MM-dd");

        if (fechaDescanso != null && fechaDescanso.getValor() != null && !fechaDescanso.getValor().isEmpty()) {
            String[] fechas = fechaDescanso.getValor().split(",");
            for (String diaF : fechas) {
                try {
                    diasFeriados.add(formatoDiaFeriado.parse(diaF));
                } catch (Exception e) {
                    System.err.println("Error día feriado: " + diaF + " - " + e.getMessage());
                }
            }
        }
        System.out.println("Fechas de descanso cargadas: " + diasFeriados);
        for (int i = 1; i < dias; i++) {
            ...
            if (diasDescanso != null) {
                ...
                for (Date diaFeriado : diasFeriados) {
                    if (esMismaFecha(cal.getTime(), diaFeriado)) {
                        System.out.println("Día feriado encontrado: " + formatoDiaFeriado.format(diaFeriado));
                        descanso = true;
                        dias++;
                        break;
                    }
...
//                    
    ...
    private boolean esMismaFecha(Date date1, Date date2) {
